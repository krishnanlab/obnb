...
class BaseSplit:
    ...
    This is an abstract class for objects used for splitting the dataset based
    on either the labels y and / or some properties of each entity, passed in as
    an 1-dimensional array x. This abstract class only defines a __repr__ magic
...
class BaseRandomSplit(BaseSortedSplit):
    ...
    Randomly generates the node properties, then use specific sorted split to
    split the dataset based on the random node properties.
