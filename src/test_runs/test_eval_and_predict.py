...
labelset_fp = data_path + 'labels/c2.cp.kegg.v6.1.entrez.BP.gsea-min10-max200-ovlppt7-jacpt5.nonred.gmt'
...
g = graph.SparseGraph.SparseGraph.from_edglst(network_fp, weighted=False, directed=False)
lsc = label.LabelsetCollection.SplitLSC.from_gmt(labelset_fp)
lsc.valsplit = valsplit.Interface.sklSKF(shuffle=True, skl_kws={'n_splits':n_split})

lsc.apply(label.Filter.EntityExistanceFilter(target_lst=g.IDmap.lst), inplace=True)
lsc.apply(label.Filter.LabelsetRangeFilterSize(min_val=min_labelset_size), inplace=True)
lsc.apply(label.Filter.NegativeFilterHypergeom(p_thresh=p_thresh), inplace=True)
print(f"After filtering, there are {len(lsc.labelIDlst)} number of effective labelsets")

scoring_obj = lambda estimator, X, y: metrics.auPRC(y, estimator.decision_function(X))
mdl = model.SupervisedLearning.LogRegCV(g, penalty='l2', solver='liblinear', max_iter=500, 
